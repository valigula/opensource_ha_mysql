...
echo "gtid_slave_pos mysql-master"
docker-compose exec -T mysql-master  mysql -u root -proot -Bse "set GLOBAL gtid_slave_pos= '0-1-7159, 1-1-1';"
sleep $SLEEPTIME
echo "gtid_slave_pos mysql-slave-1"
docker-compose exec -T mysql-slave-1  mysql -u root -proot -Bse "stop slave;set GLOBAL gtid_slave_pos= '0-1-7159, 1-1-1';start slave"
sleep $SLEEPTIME
echo "gtid_slave_pos mysql-slave-2"
docker-compose exec -T mysql-slave-2  mysql -u root -proot -Bse "stop slave; set GLOBAL gtid_slave_pos= '0-1-7159, 1-1-1'; start slave"
sleep $SLEEPTIME
